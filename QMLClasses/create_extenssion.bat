...
echo calling set_qt_flags.bat to load configurations...
if exist "set_qt_flags.bat" (
    call "set_qt_flags.bat"
) else (
    if exist "classes\set_qt_flags.bat" (
        call "classes\set_qt_flags.bat"
    ) else (
        echo WARNING: set_qt_flags.bat not found. Generating with default flags...
    )
)

echo Calling code generator...
"%RING_EXE%" %PAREC_RING% classes\qt.cf ring_qt.cpp ring_qt.ring
